...
total_deaths_by_country <- death_counts_data.df %>% 
  group_by(country) %>%
  summarize(deaths = sum(total_deaths))
...
health_budget <- merge(total_deaths_by_country,avg_health_budget.df,
                       by = c("country"))


ggplot(health_budget, aes(x = country, y = che_gdp_avg)) +
  geom_col() +
...
usa_train_df <- usa_ts %>% filter(year > 1989 & year<2016) 
...
# Forecast the death rate for 2016-2019 (validation set)
forecast <- model_fit %>%
  forecast(h = 4)

# Plot the forecasted data
# forecast %>%  autoplot(usa_ts, level = NULL) +
#   labs(title = "Total Non Com Death rate - USA",
...
autoplot(forecast_ets,color="black") +
  autolayer(usa_ts, color="blue") +
  labs(title = "Forecasting total non com death rate in USA using ETS model",
       y = "Total death rate") +
  guides(colour = guide_legend(title = "Forecast"))
...
# 
# Fit ARIMA model
arima_fit <- auto.arima(usa_train_df)

# Forecast
forecast_data <- forecast(arima_fit, h = 5)

# Convert forecast and test sets to data.frame format
forecast_df <- data.frame(forecast_data$mean)
test_df <- data.frame(usa_test_df$total_non_com_death_rate)
data.frame('Actual' = usa_test_df$total_non_com_death_rate, 'Predicted'= forecast_data$mean)


arima_metrics_usa <- postResample(pred = forecast_data$mean, obs = usa_test_df$total_non_com_death_rate)
arima_metrics_usa

acc_usa <- data.frame(Model = "Arima",
                      MAPE = mean(abs((usa_test_df$total_non_com_death_rate - forecast_data$mean)/usa_test_df$total_non_com_death_rate))*100,
                      RMSE = sqrt(mean((usa_test_df$total_non_com_death_rate - forecast_data$mean)^2))
)
acc_usa

autoplot(forecast_data,color="blue") +
  autolayer(usa_ts, color="black") +
  labs(title = "Forecasting total non com death rate in USA using Arima model",
       y = "Total death rate") +
  guides(colour = guide_legend(title = "Forecast"))

# Best time series model is : Drift model
# Lets forcast future non com death rate using the DRIFT model

forecast_data_new <- model_fit %>%
  forecast(h = 15)

forecast_data_new_filtered <- filter(forecast_data_new, .model == "Drift")
forecast_data_new_filtered

autoplot(forecast_data_new_filtered,color="blue") +
  autolayer(usa_ts, color="black") +
  labs(title = "Forecasting total non com death rate in USA using Drift model",
       y = "Total death rate") +
  guides(colour = guide_legend(title = "Forecast"))+
...
# forecast_data_new <- forecast(arima_fit, h = 10)

# forecasted_vales_usa_ts <- window(forecast_data_new$mean, start = 2020)

# forecasted_vales_usa_ts
...
# # Select year,sex,total non com death rate, population
...
usa_results <- cbind(lr_results_usa,ridge_results_usa$Predicted,lasso_results_usa$Predicted,pcr_results_usa$Predicted)

names(usa_results)[2] <- "lr_predicted"
names(usa_results)[5] <- "ridge_predicted"
names(usa_results)[6] <- "lasso_predicted"
names(usa_results)[7] <- "pcr_predicted"
usa_results <- usa_results[,-c(4)]


ggplot(usa_results, aes(x = Year)) +
  geom_line(aes(y = lr_predicted, color = "LR predicted")) +
  geom_line(aes(y = ridge_predicted, color = "Ridge predicted")) +
  geom_line(aes(y = lasso_predicted, color = "Lasso predicted")) +
  geom_line(aes(y = pcr_predicted, color = "PCR predicted")) +
  geom_line(aes(y = Actual, color = "Actual")) +
  labs(x = "Year", y = "Non Com Death Rate",title = "Non Com Death Rate - USA", color = "Line")


# Predicting future values based on trained model

col1 = c(2020,2021,2022,2023,2024,2025,2026,2027)
col2 = c(329500000
         ,331900000
         ,333287557
         ,334233854
         ...
         ,339970000
         ,341690000)
future_vals <- data.frame(year = col1, population = col2)
future.usa.death.pred <- predict(linear.fit.usa, newdata = future_vals)

future.usa.death.pred
...
#   forecast(h = 4)
# 
# accuracy_metrics_arima_brazil <- forecast_auto_arima_brazil %>%
#   accuracy(brazil_ts)
# accuracy_metrics_arima_brazil
# 
# plot(forecast_auto_arima_brazil, ylab = "ARIMA Total Non Com Death Rate - Brazil")
# lines(ma(brazil_ts, 4), col = "red")
# Fit ARIMA model
arima_fit_brazil <- auto.arima(brazil_train_df)

# Forecast
forecast_data <- forecast(arima_fit_brazil, h = 4)

# Convert forecast and test sets to data.frame format
forecast_df <- data.frame(forecast_data$mean)
test_df <- data.frame(brazil_test_df$total_non_com_death_rate)
data.frame('Actual' = brazil_test_df$total_non_com_death_rate, 'Predicted'= forecast_data$mean)


arima_metrics_brazil <- postResample(pred = forecast_data$mean, obs = brazil_test_df$total_non_com_death_rate)
arima_metrics_brazil

acc_brazil <- data.frame(Model = "Arima",
                         MAPE = mean(abs((brazil_test_df$total_non_com_death_rate - forecast_data$mean)/brazil_test_df$total_non_com_death_rate))*100,
                         RMSE = sqrt(mean((brazil_test_df$total_non_com_death_rate - forecast_data$mean)^2))
)
acc_brazil

# Best time series model is : DRIFT model
# Lets forcast future non com death rate using the arima model

forecast_data_new <- model_fit_brazil %>%
  forecast(h = 15)

forecast_data_new_filtered <- filter(forecast_data_new, .model == "Drift")
forecast_data_new_filtered

forecast_data_new <- forecast(arima_fit_brazil, h = 10)

forecasted_vales_brazil_ts <- window(forecast_data_new$mean, start = 2020)

forecasted_vales_brazil_ts
...
brazil_results <- cbind(lr_results_brazil,ridge_results_brazil$Predicted,lasso_results_brazil$Predicted,pcr_results_brazil$Predicted)

names(brazil_results)[2] <- "lr_predicted"
names(brazil_results)[5] <- "ridge_predicted"
names(brazil_results)[6] <- "lasso_predicted"
names(brazil_results)[7] <- "pcr_predicted"
brazil_results <- brazil_results[,-c(4)]

ggplot(brazil_results, aes(x = Year)) +
  geom_line(aes(y = lr_predicted, color = "LR predicted")) +
  geom_line(aes(y = ridge_predicted, color = "Ridge predicted")) +
  geom_line(aes(y = lasso_predicted, color = "Lasso predicted")) +
  geom_line(aes(y = pcr_predicted, color = "PCR predicted")) +
  geom_line(aes(y = Actual, color = "Actual")) +
  labs(x = "Year", y = "Non Com Death Rate",title = "Non Com Death Rate - Brazil", color = "Line")
...
col2 = c(213200000
         ...
         ,221142804)
future_vals <- data.frame(year = col1, population = col2)
future.brazil.death.pred <- predict(linear.fit.brazil, newdata = future_vals)

future.brazil.death.pred
